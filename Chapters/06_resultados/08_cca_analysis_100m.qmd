### Análisis de Correspondencias Canónicas con Cobertura del Suelo a 100m

Esta sección presenta los resultados del CCA usando variables de cobertura del suelo medidas a **100m de distancia** de los sitios de dunas costeras.
...
if(nrow(cca_summary_100m) > 0) {
  cca_summary_display_100m <- cca_summary_100m %>%
    mutate(
      ...
      P_Value_with_Significance = case_when(
        ...
        TRUE ~ as.character(Model_P_Value)  # Sin símbolo para no significativo (en blanco)
      )
    )
  
  knitr::kable(cca_summary_display_100m %>%
               ...
               align = c("l", "c", "c", "c", "c", "c")) %>%
    kableExtra::footnote(
      general = c("Códigos de significancia: *** p<0.001, ** p<0.01, * p<0.05, . p<0.1, (en blanco) = no significativo",
