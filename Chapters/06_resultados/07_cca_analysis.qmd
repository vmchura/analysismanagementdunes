...
El análisis combinado de todas las regiones catalanas revela los gradientes ambientales principales que estructuran las comunidades de vegetación en el conjunto de dunas costeras mediterráneas. La @fig-cca-combined muestra la ordenación CCA integrando datos de Girona, Barcelona y Tarragona para identificar los patrones ambientales generales que influyen en la distribución de especies a escala regional.
De la misma figura las flechas de "Urbano" y "Vías de Comunicación" se oponen a "Agua Dulce", "Pastizal" y "Matorral", indicando el contraste fundamental entre desarrollo humano y hábitats naturales.
```{r}
...
if(!is.null(combined_cca)) {
  ...
  # Traducir nombres de variables ambientales usando la función global
  env_scores_detailed$Variable <- translate_labels(env_scores_detailed$Variable)
