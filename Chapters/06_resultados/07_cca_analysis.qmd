...
if(nrow(cca_summary) > 0) {
  ...
  knitr::kable(cca_summary_display %>%
    ...
    kableExtra::footnote(
      general = c("Códigos de significancia: ** p<0.01, * p<0.05, . p<0.1, (en blanco) = no significativo",
...
if(!is.null(combined_sig) && !is.null(combined_sig$variables)) {
  ...
  knitr::kable(env_var_table,
               ...
               row.names = FALSE) %>%
    kableExtra::footnote(
      general = c("Códigos de significancia: *** p<0.001, ** p<0.01, * p<0.05,",
                  ". p<0.1, (en blanco) = no significativo"),
