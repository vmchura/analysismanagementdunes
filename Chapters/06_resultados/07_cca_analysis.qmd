...
# Crear tabla formateada con significancia
if(nrow(cca_summary) > 0) {
  cca_summary_display <- cca_summary %>%
    mutate(
      # Formatear valores F como texto para mostrar NA apropiadamente
      F_Value_Display = ifelse(is.na(Model_F_Value), "NA", as.character(Model_F_Value)),
      # Combinar valor P y significancia en una sola columna
      P_Value_with_Significance = case_when(
        is.na(Model_P_Value) ~ "NA",
        Model_P_Value < 0.001 ~ paste0(Model_P_Value, " ***"),
        Model_P_Value < 0.01 ~ paste0(Model_P_Value, " **"), 
        Model_P_Value < 0.05 ~ paste0(Model_P_Value, " *"),
        Model_P_Value < 0.1 ~ paste0(Model_P_Value, " ."),
        Model_P_Value >= 0.1 ~ paste0(Model_P_Value, " (n.s.)"),
      )
    )
  
  knitr::kable(cca_summary_display %>%
               select(Region, Explained_Variance_Percent, Num_Environmental_Variables, 
                     Num_Species, F_Value_Display, P_Value_with_Significance), 
    ...
    kable_styling(latex_options = c("HOLD_position", "scale_down"),
                  ...
                  position = "center") %>%
    kableExtra::footnote(
      general = c("Códigos de significancia: ** p<0.01, * p<0.05, . p<0.1, (n.s.) = no significativo",
...
if(!is.null(combined_sig) && !is.null(combined_sig$variables)) {
  env_var_table <- data.frame(
    ...
    P_Value_with_Sig = paste0(
      round(combined_sig$variables$`Pr(>F)`, 3),
      ifelse(combined_sig$variables$`Pr(>F)` < 0.001, " ***",
             ifelse(combined_sig$variables$`Pr(>F)` < 0.01, " **",
                   ifelse(combined_sig$variables$`Pr(>F)` < 0.05, " *",
                         ifelse(combined_sig$variables$`Pr(>F)` < 0.1, " .", " (n.s.)"))))
    )
  )
  ...
  knitr::kable(env_var_table,
               col.names = c("Variable Ambiental", "Valor F", "Valor P"),
               align = c("l", "c", "c"),
               format = "latex",
               booktabs = TRUE,
               row.names = FALSE) %>%
    kable_styling(latex_options = c("HOLD_position", "scale_down"),
                  ...
                  position = "center") %>%
    kableExtra::footnote(
      general = c("Códigos de significancia: *** p<0.001, ** p<0.01, * p<0.05, . p<0.1,",
                  paste0(paste(rep("\u00A0", 22), collapse = ""),"(n.s.) = no significativo")),
      general_title = "Nota:",
      escape = FALSE
    )
} else {
