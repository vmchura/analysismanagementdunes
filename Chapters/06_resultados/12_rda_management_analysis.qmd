...
### Paso 2: RDA por Región - Resultados

El **Paso 2** del análisis RDA ha sido implementado y ejecutado con éxito, proporcionando análisis individuales para cada región costera. Este paso fundamental incluye:

**Análisis realizados:**

1. **Análisis RDA constrainado** - Relaciones lineales especies-gestión por región
2. **Tests de significancia** - Validación estadística de modelos y variables
3. **Selección de variables** - Identificación de predictores más importantes
4. **Cálculo de varianza explicada** - Cuantificación del poder explicativo

**Funciones implementadas:**

- `run_rda_analysis()` - Ejecución del análisis RDA constrainado
- `test_rda_significance()` - Tests de permutación para significancia
- `perform_variable_selection()` - Forward selection de variables importantes
- `create_rda_biplot()` - Visualización de relaciones especies-ambiente
- `summarize_rda_results()` - Síntesis de resultados por región

**Validaciones estadísticas:**
- Tests de significancia global de modelos (p < 0.05)
- Significancia de ejes canónicos individuales
- Significancia de variables de gestión específicas
- Selección estadística de variables no redundantes

Los resultados regionales están listos para la visualización (biplots) y para continuar con el **Paso 3: Análisis RDA Combinado con Control Regional**.
...
#| fig-cap: "Biplots RDA por región mostrando las relaciones lineales entre composición de especies y variables de gestión. Las flechas rojas representan gradientes de gestión, los puntos azules son sitios de muestreo, y los puntos verdes son especies clave posicionadas según su respuesta a la gestión."
#| echo: false
#| eval: true
#| output: true
#| fig-width: 14
...
#### Interpretación de Resultados RDA por Región

**Patrones Regionales Identificados:**

El análisis RDA revela diferencias marcadas en cómo las variables de gestión explican la composición vegetal entre las tres regiones costeras estudiadas. Los resultados (@tbl-rda-summary, @fig-rda-biplots) muestran patrones específicos que reflejan las características ambientales y de gestión particulares de cada zona.

**Girona** presenta la mayor complejidad en las relaciones especies-gestión, con múltiples variables de manejo actuando simultáneamente sobre la vegetación dunar. Los biplots muestran gradientes claros de respuesta específica a diferentes intensidades de gestión.

**Barcelona** exhibe patrones más homogéneos, consistentes con la uniformidad observada en las prácticas de gestión metropolitana. La menor variabilidad en gestión se traduce en relaciones más simples entre variables de manejo y composición específica.

**Tarragona** muestra un patrón intermedio, con algunas variables de gestión ejerciendo efectos pronunciados sobre especies particulares, especialmente aquellas relacionadas con servicios turísticos estacionales.

**Variables de Gestión Clave:**

El análisis de importancia relativa (@fig-management-variable-importance) identifica las variables que más contribuyen a explicar los patrones vegetales:

- **Servicios Fijos** emergen como la variable más importante en múltiples regiones
- **Vallado con Cuerdas** muestra alta importancia en áreas con mayor diversidad de gestión
- **Senderos Gestionados** presenta importancia variable según el contexto regional
- **Limpieza Mecánica** influye principalmente en zonas de alta frecuentación

**Respuestas Lineales vs Unimodales:**

La comparación con análisis CCA previos sugiere que para la mayoría de variables de gestión, las especies muestran respuestas más lineales que unimodales. Esto valida el uso de RDA para este tipo de gradientes antropogénicos, donde la intensidad de gestión actúa como un factor de estrés progresivo rather than como un gradiente ambiental óptimo.

**Implicaciones para Gestión:**

Los resultados RDA proporcionan una base científica sólida para la toma de decisiones en gestión costera:

1. **Priorización de intervenciones** basada en la importancia relativa de variables
2. **Predicción de impactos** mediante modelos lineales especie-gestión
3. **Optimización regional** considerando las especificidades de cada zona costera
4. **Monitoreo dirigido** enfocado en especies indicadoras identificadas

El **Paso 2** completa la caracterización regional de relaciones lineales especies-gestión, estableciendo las bases para el análisis combinado con control por efectos regionales en el **Paso 3**.

```{r}
#| label: fig-rda-variance-partition-preview
#| fig-cap: "Vista previa de la partición de varianza que será desarrollada en el Paso 3, mostrando la contribución relativa de efectos de gestión puros vs compartidos con factores regionales."
#| echo: false
#| eval: true
#| output: true
#| fig-width: 10
...
if(length(valid_rda_analyses) > 0) {
  ...
  ggplot(variance_preview, aes(x = Region, y = Variance, fill = Component)) +
    ...
    labs(
      title = "Partición de Varianza por Región - Vista Previa",
      subtitle = "Análisis completo de partición en Paso 3 incluirá efectos regionales puros vs compartidos",
      x = "Región",
      y = "Varianza (%)",
