...
#| tbl-cap: "Resumen de resultados RDA por región mostrando varianza explicada, significancia estadística y variables de gestión seleccionadas."
#| echo: false
#| eval: true
#| output: true
...
if(exists("rda_summary") && nrow(rda_summary) > 0) {
  ...
  knitr::kable(rda_summary_display,
               ...
               caption = "Códigos de significancia: *** p<0.001, ** p<0.01, * p<0.05, ns = no significativo") %>%
...
#| fig-cap: "Biplots RDA por región mostrando las relaciones lineales entre composición de especies y variables de gestión."
#| echo: false
#| eval: true
#| output: true
#| fig-width: 11
...
#| fig-cap: "Importancia relativa de variables de gestión en cada región basada en eigenvalues canónicos del análisis RDA. Las variables con mayor contribución aparecen como las más importantes para explicar patrones de vegetación."
#| echo: false
#| eval: true
#| output: true
#| fig-width: 12
...
perform_variance_partition <- function(combined_rda_result) {
  ...
  tryCatch({
    varpart_result <- varpart(
      combined_rda_result$species_matrix,
      combined_rda_result$management_matrix,
      combined_rda_result$region_indicators
    )
    ...
    # Componentes:
    # [a] = Gestión pura
    # [b] = Compartida entre gestión y región  
    # [c] = Región pura
    # [d] = Residual (no explicada)
    ...
    if (interactive()) cat("Componentes de varianza:\n")
    if (interactive()) cat("  - Gestión pura:", round(pure_management, 2), "%\n")
    if (interactive()) cat("  - Compartida:", round(shared_variance, 2), "%\n")
    if (interactive()) cat("  - Región pura:", round(pure_region, 2), "%\n")
    if (interactive()) cat("  - Residual:", round(residual, 2), "%\n")
...
```{r}
#| label: tbl-combined-rda-summary
#| tbl-cap: "Resumen del análisis RDA combinado mostrando la partición de varianza entre efectos de gestión, región y su interacción."
#| echo: false
#| eval: true
#| output: true

if(!is.null(combined_rda_result)) {
  # Crear tabla resumen del análisis combinado
  combined_summary <- data.frame(
    Componente = c(
      "Solo Gestión",
      "Solo Región", 
      "Gestión + Región",
      "Gestión | Región (Condicional)"
    ),
    Varianza_Explicada = c(
      paste0(round(combined_rda_result$variance_management_only, 1), "%"),
      paste0(round(combined_rda_result$variance_region_only, 1), "%"),
      paste0(round(combined_rda_result$variance_full, 1), "%"),
      paste0(round(combined_rda_result$variance_conditional, 1), "%")
    ),
    Descripcion = c(
      "Efecto de gestión sin control regional",
      "Efecto regional sin variables de gestión",
      "Efecto conjunto de gestión y región",
      "Efecto puro de gestión controlando región"
    ),
    stringsAsFactors = FALSE
  )
  
  knitr::kable(combined_summary,
               col.names = c("Componente", "Varianza Explicada", "Descripción"),
               align = c("l", "c", "l")) %>%
    kableExtra::kable_styling(latex_options = c("hold_position", "scale_down"))
} else {
  if (interactive()) cat("No se pudo generar tabla resumen del análisis RDA combinado.\n")
}
```

```{r}
#| label: fig-combined-rda-triplot  
#| fig-cap: "Triplot RDA combinado mostrando efectos puros de gestión controlando por región. Los sitios están coloreados por región, las flechas rojas muestran gradientes de gestión, y los puntos verdes indican especies características. Las elipses muestran la agrupación regional en el espacio ordenado."
#| echo: false
#| eval: true
#| output: true
#| fig-width: 14
...
if(!is.null(combined_rda_result)) {
  # Crear triplot RDA combinado
  p_combined_triplot <- create_combined_rda_triplot(combined_rda_result)
  
  if(!is.null(p_combined_triplot)) {
    print(p_combined_triplot)
    
    # Guardar el gráfico
    ggsave(here("figures","rda_combined_triplot.png"), p_combined_triplot,
           width = 14, height = 10)
  }
} else {
  if (interactive()) cat("No se pudo crear triplot RDA combinado.\n")
}
```

```{r}
#| label: fig-variance-partition
#| fig-cap: "Partición de varianza mostrando los componentes puros y compartidos entre gestión y región. La partición cuantifica qué proporción de varianza en composición de especies se debe exclusivamente a gestión, exclusivamente a región, a efectos compartidos, o permanece sin explicar."
#| echo: false
#| eval: true
#| output: true
#| fig-width: 12
...
if(!is.null(variance_partition_result)) {
  ...
  # Crear también diagrama tipo Venn conceptual
  venn_data <- data.frame(
    x = c(1, 1.8, 1.4),
    y = c(1, 1, 1.5),
    Componente = c("Gestión", "Región", "Compartida"),
    Valor = c(
      variance_partition_result$pure_management,
      variance_partition_result$pure_region,
      variance_partition_result$shared_variance
  ...
  p_venn <- ggplot(venn_data, aes(x = x, y = y)) +
    geom_point(aes(size = Valor, color = Componente), alpha = 0.7) +
    geom_text(aes(label = paste(Componente, "\n", round(Valor, 1), "%")),
              color = "black", fontface = "bold", size = 4, vjust = -1.5) +
    ...
    theme_void() +
    labs(
      title = "Efectos Puros vs Compartidos",
      subtitle = paste("Residual:", round(variance_partition_result$residual, 1), "%")
    ) +
    theme(
  ...
  combined_partition <- p_partition + p_venn
  print(combined_partition)
  
  # Guardar gráfico
  ggsave(here("figures","rda_variance_partition.png"), combined_partition,
         width = 14, height = 8)
} else {
  if (interactive()) cat("No se pudo crear visualización de partición de varianza.\n")
}
```
...
# --- VALIDACIÓN Y ROBUSTEZ DEL ANÁLISIS RDA COMBINADO ---

# Función para probar significancia del análisis RDA combinado
test_combined_rda_significance <- function(combined_rda_result) {
  if(is.null(combined_rda_result)) {
    return(NULL)
  }
  
  if (interactive()) cat("=== PROBANDO SIGNIFICANCIA RDA COMBINADO ===\n")
  
  tryCatch({
    # Test de significancia del modelo de gestión pura (condicional)
    sig_conditional <- anova(combined_rda_result$rda_conditional, permutations = 999)
    
    # Test de significancia del modelo completo
    sig_full <- anova(combined_rda_result$rda_full, permutations = 999)
    
    # Test de significancia por ejes del modelo condicional
    sig_axes <- anova(combined_rda_result$rda_conditional, by = "axis", permutations = 999)
    
    # Test de significancia por términos del modelo condicional
    sig_terms <- anova(combined_rda_result$rda_conditional, by = "terms", permutations = 999)
    
    if (interactive()) cat("Significancia modelo condicional: p =", round(sig_conditional$`Pr(>F)`[1], 3), "\n")
    if (interactive()) cat("Significancia modelo completo: p =", round(sig_full$`Pr(>F)`[1], 3), "\n")
    
    return(list(
      conditional = sig_conditional,
      full = sig_full,
      axes = sig_axes,
      terms = sig_terms
    ))
    ...
    warning(paste("Error en test de significancia RDA combinado:", e$message))
    return(NULL)
  })
}

# Función para comparar métodos (RDA vs CCA vs NMDS)
compare_ordination_methods <- function(combined_rda_result, rda_datasets) {
  if(is.null(combined_rda_result)) {
    return(NULL)
  }
  
  if (interactive()) cat("=== COMPARANDO MÉTODOS DE ORDENACIÓN ===\n")
  
  tryCatch({
    # Ejecutar CCA con las mismas especies pero usando Chi-squared distance
    cca_comparison <- cca(combined_rda_result$species_matrix ~ ., 
                         data = combined_rda_result$management_matrix)
    
    # Ejecutar NMDS para comparación
    nmds_comparison <- metaMDS(combined_rda_result$species_matrix, 
                              distance = "bray", k = 2, trymax = 100)
    
    # Calcular correlaciones entre ordenaciones (Procrustes)
    # RDA vs CCA
    procrustes_rda_cca <- procrustes(scores(combined_rda_result$rda_management_only, display = "sites"),
                                    scores(cca_comparison, display = "sites"))
    
    # RDA vs NMDS
    procrustes_rda_nmds <- procrustes(scores(combined_rda_result$rda_management_only, display = "sites"),
                                     scores(nmds_comparison, display = "sites"))
    
    if (interactive()) cat("Correlación RDA-CCA (Procrustes):", round(sqrt(1 - procrustes_rda_cca$ss), 3), "\n")
    if (interactive()) cat("Correlación RDA-NMDS (Procrustes):", round(sqrt(1 - procrustes_rda_nmds$ss), 3), "\n")
    
    return(list(
      cca_comparison = cca_comparison,
      nmds_comparison = nmds_comparison,
      procrustes_rda_cca = procrustes_rda_cca,
      procrustes_rda_nmds = procrustes_rda_nmds
    ))
    
  }, error = function(e) {
    warning(paste("Error en comparación de métodos:", e$message))
    return(NULL)
  })
}

# Función para análisis de sensibilidad (bootstrap)
perform_sensitivity_analysis <- function(combined_rda_result, n_bootstrap = 100) {
  if(is.null(combined_rda_result)) {
    return(NULL)
  }
  
  if (interactive()) cat("=== ANÁLISIS DE SENSIBILIDAD (BOOTSTRAP) ===\n")
  
  tryCatch({
    n_sites <- nrow(combined_rda_result$species_matrix)
    bootstrap_results <- matrix(NA, nrow = n_bootstrap, ncol = 4)
    colnames(bootstrap_results) <- c("Var_Management", "Var_Region", "Var_Full", "Var_Conditional")
    
    for(i in 1:n_bootstrap) {
      # Muestreo bootstrap de sitios
      boot_indices <- sample(1:n_sites, size = n_sites, replace = TRUE)
      
      boot_species <- combined_rda_result$species_matrix[boot_indices, ]
      boot_management <- combined_rda_result$management_matrix[boot_indices, ]
      boot_region <- factor(combined_rda_result$region_indicators[boot_indices])
      
      # RDA bootstrap
      boot_env <- data.frame(boot_management, Region = boot_region)
      
      boot_rda_mgmt <- rda(boot_species ~ ., data = boot_management)
      boot_rda_region <- rda(boot_species ~ Region, data = boot_env)
      boot_rda_full <- rda(boot_species ~ ., data = boot_env)
      boot_rda_cond <- rda(boot_species ~ . + Condition(Region), data = boot_env)
      
      total_inertia <- sum(eigenvals(boot_rda_mgmt))
      
      bootstrap_results[i, 1] <- sum(eigenvals(boot_rda_mgmt)[1:min(length(eigenvals(boot_rda_mgmt)), ncol(boot_management))]) / total_inertia * 100
      bootstrap_results[i, 2] <- sum(eigenvals(boot_rda_region)[1:1]) / total_inertia * 100
      bootstrap_results[i, 3] <- sum(eigenvals(boot_rda_full)[1:min(length(eigenvals(boot_rda_full)), ncol(boot_env)-1)]) / total_inertia * 100
      bootstrap_results[i, 4] <- sum(eigenvals(boot_rda_cond)) / total_inertia * 100
    }
    
    # Calcular intervalos de confianza
    ci_results <- apply(bootstrap_results, 2, function(x) {
      c(mean = mean(x, na.rm = TRUE),
        lower = quantile(x, 0.025, na.rm = TRUE),
        upper = quantile(x, 0.975, na.rm = TRUE))
    })
    
    if (interactive()) cat("Intervalos de confianza 95% (bootstrap):\n")
    for(i in 1:ncol(ci_results)) {
      if (interactive()) cat(sprintf("  %s: %.1f%% [%.1f%% - %.1f%%]\n", 
                  colnames(ci_results)[i],
                  ci_results["mean", i],
                  ci_results["lower", i],
                  ci_results["upper", i]))
    }
    ...
    warning(paste("Error en análisis de sensibilidad:", e$message))
    return(NULL)
  })
}

# Ejecutar validación y análisis de robustez
if(!is.null(combined_rda_result)) {
  if (interactive()) cat("\n=== VALIDACIÓN Y ROBUSTEZ ===\n")
  
  # Probar significancia
  combined_rda_significance <- test_combined_rda_significance(combined_rda_result)
  
  # Comparar métodos
  method_comparison <- compare_ordination_methods(combined_rda_result, valid_rda_datasets)
  
  # Análisis de sensibilidad (con menos bootstrap para velocidad)
  sensitivity_analysis <- perform_sensitivity_analysis(combined_rda_result, n_bootstrap = 50)
...
```{r}
#| label: tbl-combined-rda-significance
#| tbl-cap: "Tests de significancia para el análisis RDA combinado, incluyendo significancia global del modelo, ejes individuales y términos de gestión."
#| echo: false
#| eval: true
#| output: true

if(!is.null(combined_rda_significance)) {
  # Crear tabla de significancia
  sig_summary <- data.frame(
    Test = c(
      "Modelo Condicional (Gestión | Región)",
      "Modelo Completo (Gestión + Región)",
      "Primer Eje RDA",
      "Segundo Eje RDA"
    ),
    Df = c(
      combined_rda_significance$conditional$Df[1],
      combined_rda_significance$full$Df[1],
      if(!is.null(combined_rda_significance$axes)) combined_rda_significance$axes$Df[1] else NA,
      if(!is.null(combined_rda_significance$axes)) combined_rda_significance$axes$Df[2] else NA
    ),
    F_Value = c(
      round(combined_rda_significance$conditional$F[1], 2),
      round(combined_rda_significance$full$F[1], 2),
      if(!is.null(combined_rda_significance$axes)) round(combined_rda_significance$axes$F[1], 2) else NA,
      if(!is.null(combined_rda_significance$axes)) round(combined_rda_significance$axes$F[2], 2) else NA
    ),
    P_Value = c(
      round(combined_rda_significance$conditional$`Pr(>F)`[1], 3),
      round(combined_rda_significance$full$`Pr(>F)`[1], 3),
      if(!is.null(combined_rda_significance$axes)) round(combined_rda_significance$axes$`Pr(>F)`[1], 3) else NA,
      if(!is.null(combined_rda_significance$axes)) round(combined_rda_significance$axes$`Pr(>F)`[2], 3) else NA
    ),
    Significance = c(
      ifelse(combined_rda_significance$conditional$`Pr(>F)`[1] < 0.001, "***",
             ifelse(combined_rda_significance$conditional$`Pr(>F)`[1] < 0.01, "**",
                   ifelse(combined_rda_significance$conditional$`Pr(>F)`[1] < 0.05, "*", "ns"))),
      ifelse(combined_rda_significance$full$`Pr(>F)`[1] < 0.001, "***",
             ifelse(combined_rda_significance$full$`Pr(>F)`[1] < 0.01, "**",
                   ifelse(combined_rda_significance$full$`Pr(>F)`[1] < 0.05, "*", "ns"))),
      if(!is.null(combined_rda_significance$axes)) {
        ifelse(combined_rda_significance$axes$`Pr(>F)`[1] < 0.001, "***",
               ifelse(combined_rda_significance$axes$`Pr(>F)`[1] < 0.01, "**",
                     ifelse(combined_rda_significance$axes$`Pr(>F)`[1] < 0.05, "*", "ns")))
      } else NA,
      if(!is.null(combined_rda_significance$axes)) {
        ifelse(combined_rda_significance$axes$`Pr(>F)`[2] < 0.001, "***",
               ifelse(combined_rda_significance$axes$`Pr(>F)`[2] < 0.01, "**",
                     ifelse(combined_rda_significance$axes$`Pr(>F)`[2] < 0.05, "*", "ns")))
      } else NA
    ),
    stringsAsFactors = FALSE
  )
  ...
  sig_summary <- sig_summary[!is.na(sig_summary$F_Value), ]
  
  knitr::kable(sig_summary,
               col.names = c("Tipo de Test", "gl", "Valor F", "Valor p", "Sig."),
               align = c("l", "c", "c", "c", "c"),
               caption = "Códigos de significancia: *** p<0.001, ** p<0.01, * p<0.05, ns = no significativo") %>%
    kableExtra::kable_styling(latex_options = c("hold_position", "scale_down"))
} else {
  if (interactive()) cat("Tests de significancia no disponibles.\n")
}
```

```{r}
#| label: fig-method-comparison
#| fig-cap: "Comparación entre métodos de ordenación (RDA vs CCA vs NMDS) usando análisis Procrustes para evaluar la congruencia entre diferentes aproximaciones metodológicas."
#| echo: false
#| eval: true
#| output: true
#| fig-width: 12
...
if(!is.null(method_comparison)) {
  # Crear gráfico de comparación Procrustes RDA vs CCA
  rda_scores <- scores(combined_rda_result$rda_management_only, display = "sites")
  cca_scores <- scores(method_comparison$cca_comparison, display = "sites")
  nmds_scores <- scores(method_comparison$nmds_comparison, display = "sites")
  
  # Datos para gráficos Procrustes
  procrustes_rda_cca_data <- data.frame(
    RDA1 = rda_scores[, 1],
    RDA2 = rda_scores[, 2],
    CCA1 = method_comparison$procrustes_rda_cca$Yrot[, 1],
    CCA2 = method_comparison$procrustes_rda_cca$Yrot[, 2],
    Region = combined_rda_result$region_indicators
  )
  
  procrustes_rda_nmds_data <- data.frame(
    RDA1 = rda_scores[, 1],
    RDA2 = rda_scores[, 2],
    NMDS1 = method_comparison$procrustes_rda_nmds$Yrot[, 1],
    NMDS2 = method_comparison$procrustes_rda_nmds$Yrot[, 2],
    Region = combined_rda_result$region_indicators
  )
  
  # Gráfico RDA vs CCA
  p_rda_cca <- ggplot(procrustes_rda_cca_data, aes(x = RDA1, y = CCA1, color = Region)) +
    geom_point(size = 3, alpha = 0.7) +
    geom_smooth(method = "lm", se = TRUE, color = "black", linetype = "dashed") +
    scale_color_manual(values = PROVINCE_COLORS) +
    theme_minimal() +
    labs(
      title = "RDA vs CCA (Procrustes)",
      subtitle = paste("Correlación:", round(sqrt(1 - method_comparison$procrustes_rda_cca$ss), 3)),
      x = "RDA - Eje 1",
      y = "CCA - Eje 1 (rotado)",
      color = "Región"
    ) +
  ...
  # Gráfico RDA vs NMDS
  p_rda_nmds <- ggplot(procrustes_rda_nmds_data, aes(x = RDA1, y = NMDS1, color = Region)) +
    geom_point(size = 3, alpha = 0.7) +
    geom_smooth(method = "lm", se = TRUE, color = "black", linetype = "dashed") +
    scale_color_manual(values = PROVINCE_COLORS) +
    theme_minimal() +
    labs(
      title = "RDA vs NMDS (Procrustes)",
      subtitle = paste("Correlación:", round(sqrt(1 - method_comparison$procrustes_rda_nmds$ss), 3)),
      x = "RDA - Eje 1", 
      y = "NMDS - Eje 1 (rotado)",
      color = "Región"
    ) +
  ...
  # Combinar gráficos
  combined_comparison <- p_rda_cca + p_rda_nmds
  print(combined_comparison)
  
  # Guardar gráfico
  ggsave(here("figures","rda_method_comparison.png"), combined_comparison,
         width = 12, height = 6)
} else {
  if (interactive()) cat("Comparación de métodos no disponible.\n")
}
```

```{r}
#| label: fig-sensitivity-analysis
#| fig-cap: "Análisis de sensibilidad mediante bootstrap mostrando intervalos de confianza para los componentes de varianza explicada. Los intervalos indican la robustez de las estimaciones frente a variaciones en el muestreo."
#| echo: false
#| eval: true
#| output: true
#| fig-width: 10
...
if(!is.null(sensitivity_analysis)) {
  # Crear data frame para visualización de intervalos de confianza
  ci_data <- data.frame(
    Component = c("Solo Gestión", "Solo Región", "Gestión + Región", "Gestión | Región"),
    Mean = sensitivity_analysis$confidence_intervals["mean", ],
    Lower = sensitivity_analysis$confidence_intervals["lower", ],
    Upper = sensitivity_analysis$confidence_intervals["upper", ],
    stringsAsFactors = FALSE
  )
  
  # Gráfico de intervalos de confianza
  ggplot(ci_data, aes(x = reorder(Component, Mean), y = Mean)) +
    ...
    theme_minimal() +
    labs(
      title = "Intervalos de Confianza Bootstrap (95%)",
      subtitle = "Robustez de estimaciones de varianza explicada",
      x = "Componente del Modelo",
      y = "Varianza Explicada (%)",
      caption = "Basado en 50 muestras bootstrap"
    ) +
    theme(
      plot.title = element_text(face = "bold"),
      axis.title = element_text(size = 12),
  ...
  # Guardar gráfico
  ggsave(here("figures","rda_sensitivity_analysis.png"), width = 10, height = 6)
} else {
  if (interactive()) cat("Análisis de sensibilidad no disponible.\n")
}
```