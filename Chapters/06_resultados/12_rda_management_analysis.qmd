...
## Análisis de Redundancia (RDA) para Variables de Gestión

El análisis de redundancia constituye una técnica de ordenación constrainada que permite evaluar directamente la influencia de variables de gestión sobre la composición de especies vegetales. A diferencia de los métodos de ordenación no constrainados, el RDA modela explícitamente las relaciones lineales entre la matriz de especies y las variables ambientales explicativas, proporcionando una medida cuantitativa de la varianza explicada por factores de manejo.

Este análisis se estructura en tres fases complementarias: primero, se ejecutan análisis RDA independientes para cada región para identificar patrones específicos; segundo, se evalúa la importancia relativa de cada variable de gestión; y tercero, se implementa un análisis RDA combinado que controla por efectos regionales, permitiendo separar los efectos puros de gestión de aquellos confundidos con diferencias geográficas.
...
La @tbl-rda-summary presenta los resultados principales del análisis de redundancia (RDA) aplicado por separado a cada región. El análisis reveló diferencias importantes en la capacidad explicativa de las variables de gestión sobre la composición de especies vegetales. Girona muestra la menor varianza explicada con un 44.8%, seguido de Tarragona con 50.6% y Barcelona con 100%. La significancia estadística indica que las relaciones observadas entre gestión y vegetación son estadísticamente válidas en Girona, mientras que Tarragona no alcanza significancia estadística. El número de variables de gestión seleccionadas mediante forward selection varió entre regiones, sugiriendo diferentes patrones de influencia de las prácticas de manejo.

```{r}
#| label: tbl-rda-summary
#| tbl-cap: "Resumen de resultados del análisis de redundancia (RDA) por región mostrando varianza explicada, significancia estadística y número de variables de gestión utilizadas y seleccionadas."
#| echo: false
#| eval: true
#| output: true
...
Los biplots RDA de la @fig-rda-biplots visualizan las relaciones entre la composición de especies y las variables de gestión para cada región. En estos gráficos, las flechas rojas representan las variables de gestión y su longitud indica la importancia relativa, mientras que los puntos verdes muestran las especies más influyentes y los puntos azules representan las playas muestreadas. En Girona, las variables 'Senderos Gestionados' y 'Protección del Sistema' emergen como los factores de gestión más determinantes. Barcelona presenta un patrón más concentrado con 'Senderos Gestionados' y 'Vallado con Cuerdas' como principales factores estructuradores. Tarragona muestra el patrón más complejo, con múltiples variables de gestión influyendo de manera similar en la composición vegetal.

```{r}
#| label: fig-rda-biplots
#| fig-cap: "Biplots del análisis de redundancia (RDA) por región. Las flechas rojas indican variables de gestión (longitud = importancia), puntos verdes muestran especies principales y puntos azules representan playas. La disposición espacial refleja las asociaciones entre gestión y composición vegetal."
#| echo: false
#| eval: true
#| output: true
#| fig-width: 11
...
La @fig-management-variable-importance cuantifica la importancia relativa de cada variable de gestión dentro de cada región. En Girona, 'Senderos Gestionados' emerge como la variable más influyente, seguida de 'Protección del Sistema' y 'Protección IUCN'. En Barcelona, 'Senderos Gestionados' también domina, pero con contribuciones más equilibradas de 'Vallado con Cuerdas' y otras variables. Tarragona presenta el patrón más homogéneo, con múltiples variables de gestión mostrando importancias similares, lo que sugiere un sistema más complejo de interacciones entre diferentes prácticas de manejo.

```{r}
#| label: fig-rda-variance-partition-preview
#| fig-cap: "Varianza explicada por factores de gestión en cada región mediante análisis RDA individual. Barcelona muestra la mayor capacidad explicativa (100%), seguida de Tarragona (50.6%) y Girona (44.8%). La varianza no explicada sugiere la importancia de otros factores no medidos."
#| echo: false
#| eval: true
#| output: true
#| fig-width: 10
...
if(length(valid_rda_analyses) > 0) {
  # Crear vista previa de partición de varianza usando datos actuales
  variance_preview <- data.frame(
    Region = names(valid_rda_analyses),
    Management_Variance = sapply(valid_rda_analyses, function(x) x$explained_variance),
    Unexplained_Variance = sapply(valid_rda_analyses, function(x) 100 - x$explained_variance),
    stringsAsFactors = FALSE
  ) %>%
    pivot_longer(cols = c(Management_Variance, Unexplained_Variance),
                 names_to = "Component",
                 values_to = "Variance") %>%
    mutate(
      Component = case_when(
        Component == "Management_Variance" ~ "Gestión (Explicada)",
        Component == "Unexplained_Variance" ~ "No Explicada",
        TRUE ~ Component
      )
    )

  ggplot(variance_preview, aes(x = Region, y = Variance, fill = Component)) +
    ...
    labs(
      title = "Varianza Explicada por Gestión por Región",
      subtitle = "Proporción de varianza en composición vegetal atribuible a variables de gestión",
      x = "Región",
      y = "Varianza (%)",
      fill = "Componente de Varianza"
    ) +
  ...
  # Guardar gráfico
  ggsave(here("figures","rda_variance_partition_preview.png"), width = 10, height = 6)
} else {
  if (interactive()) cat("No hay datos RDA para crear vista previa de partición de varianza.\n")
}

```

La @fig-rda-variance-partition-preview compara la capacidad explicativa de las variables de gestión entre regiones cuando se analizan por separado. Barcelona presenta el máximo poder explicativo (100%), sugiriendo que las variables de gestión medidas capturan completamente la variación en composición vegetal de esta región. Tarragona muestra una capacidad explicativa intermedia (50.6%), mientras que Girona presenta la menor explicación (44.8%). Estas diferencias regionales podrían reflejar distintos grados de impacto de la gestión, diferentes historias de manejo, o la presencia de factores ambientales no medidos que varían en importancia entre regiones.
...
#| fig-cap: "Análisis de redundancia combinado controlando por efectos regionales. El modelo permite separar los efectos puros de gestión de aquellos confundidos con diferencias regionales, proporcionando una evaluación más precisa del impacto de las prácticas de manejo."
#| echo: false
#| eval: true
#| output: true
#| fig-width: 10
...
El análisis RDA combinado permite evaluar los efectos de gestión controlando por diferencias regionales inherentes. Este enfoque más robusto separa la varianza atribuible exclusivamente a prácticas de gestión de aquella que podría estar confundida con características regionales (clima, geología, historia de uso). Los resultados de este análisis combinado proporcionan una evaluación más precisa del verdadero impacto de las variables de manejo sobre la composición vegetal, eliminando el sesgo que podría introducir la agrupación geográfica de los datos.


## Síntesis de Resultados del Análisis RDA

Los análisis de redundancia revelan patrones complejos en la influencia de la gestión sobre la vegetación dunar. A nivel regional, las variables de gestión explican entre 44.8% y 100% de la varianza en composición vegetal, con Barcelona mostrando la mayor capacidad explicativa. Sin embargo, el análisis combinado que controla por efectos regionales reduce drásticamente la contribución atribuible exclusivamente a gestión (4.3%), indicando que gran parte de la varianza aparentemente explicada por gestión en los análisis regionales está confundida con características geográficas inherentes.

Los 'Senderos Gestionados' emergen consistentemente como la variable de gestión más influyente across regiones, seguidos por medidas de protección y control de acceso. La dominancia de la varianza no explicada (88.3%) sugiere que factores no medidos—como microhábitat, historia de perturbaciones, procesos estocásticos, o variables ambientales de escala fina—ejercen una influencia fundamental sobre los patrones de vegetación dunar que supera el efecto de las prácticas de gestión evaluadas.

La @fig-variance-partition cuantifica la contribución relativa de los efectos de gestión versus los efectos regionales en la variación de la composición vegetal. Esta partición de varianza revela que únicamente el 4.3% de la varianza puede atribuirse exclusivamente a factores de gestión, mientras que el 9% corresponde a efectos compartidos entre gestión y región. La región como factor puro no contribuye significativamente, y el 88.3% de la varianza permanece sin explicar por estos factores. Este resultado sugiere que otros factores no medidos (como condiciones microambientales, historia de perturbaciones, o procesos estocásticos) ejercen una influencia dominante sobre los patrones de vegetación dunar.

```{r}
#| label: fig-variance-partition
#| fig-cap: "Partición de varianza mediante análisis de redundancia combinado. Descomposición cuantitativa de la varianza en composición vegetal atribuible exclusivamente a gestión (4.3%), exclusivamente a región (0%), efectos compartidos (9%) y factores no explicados (88.3%)."
#| echo: false
#| eval: true
#| output: true
#| fig-width: 12
