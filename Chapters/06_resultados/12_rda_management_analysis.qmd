...
if(exists("rda_summary") && nrow(rda_summary) > 0) {
  # Crear tabla formateada con P valores y significancia combinados
  rda_summary_display <- rda_summary %>%
    ...
    select(Region, Variance_Total, Variance_RDA1, Variance_RDA2, Variables_Used, F_Value_Display, P_Value_with_Significance)

  knitr::kable(rda_summary_display,
               col.names = c("Región", "Var. Total", "RDA1", "RDA2", "Var. (Total/Selec.)", "F", "Valor P"),
               align = c("l", "c", "c", "c", "c", "c", "c")) %>%
    kableExtra::kable_styling(latex_options = c("hold_position", "scale_down")) %>%
    kableExtra::footnote(general = c("Códigos de significancia: *** p<0.001, ** p<0.01, * p<0.05, . p<0.1,",
                                     paste0(paste(rep("\u00A0", 22), collapse = ""),"(n.s.) = no significativo"),
                                     "NA = No aplicable debido a sobreajuste estadístico (Barcelona: datos insuficientes",
                                     "para test de significancia)"))
} else {
