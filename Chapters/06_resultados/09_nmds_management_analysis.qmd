...
Esta sección presenta los resultados de un análisis de NMDS para examinar cómo las prácticas de gestión de playas influyen en la composición de vegetación de dunas costeras.
...
La distribución de las prácticas de gestión varía considerablemente entre las tres regiones costeras estudiadas (@fig-management-region-distribution, @tbl-management-summary). Barcelona presenta un patrón muy homogéneo, con el 100.0% de las parcelas bajo gestión de baja intensidad. En contraste, Girona muestra la mayor diversidad: 12.7% de parcelas sin gestión, 44.1% con gestión baja y 43.2% con gestión moderada. Tarragona mantiene una posición intermedia con 60.3% de gestión baja y 39.7% de gestión moderada.
...
ggplot(management_practices, aes(x = Region, y = mean_score, fill = practice)) +
  ...
  labs(
    ...
    subtitle = "Puntuaciones medias en escala 0-5. Las barras negras muestran error estándar (SE), menor barra corresponde a mayor precisión",
    x = "Región",
...
Las curvas de respuesta específica revelan patrones diferenciados a lo largo del gradiente de gestión (@fig-management-gradient-analysis). Especies como Ammophila arenaria (barrón) y Artemisia campestris muestran abundancias máximas en gestión baja (intensidad 1-2), mientras que Plantago crassifolia y Juncus acutus (junco marino) aumentan progresivamente hacia gestión alta (intensidad 5). Arthrocnemum fruticosum presenta un pico en gestión moderada (intensidad 3), sugiriendo respuestas específicas de tolerancia a la perturbación.

```{r}
#| label: fig-management-gradient-analysis
#| fig-cap: "Respuestas específicas al gradiente de gestión, mostrando tres patrones principales: especies que prefieren gestión baja, especies tolerantes a gestión alta, y especies con óptimos en gestión moderada."
#| echo: false
#| eval: true
#| output: true
#| fig-width: 14
...
ggplot(gradient_data, aes(x = management_rounded, y = mean_abundance, color = species)) +
  ...
  labs(
    ...
    subtitle = "Respuesta de abundancia de especies a la escala ordinal de gestión (0-5). Las barras verticales muestran error estándar (SE), menor barra corresponde a mayor precisión",
...
La matriz de correlación revela patrones de asociación entre prácticas de gestión (@fig-management-correlation-matrix). Senderos gestionados muestra las correlaciones más altas con vallado con cuerdas (0.78) y limpieza mecánica (0.77), indicando que estas prácticas tienden a implementarse conjuntamente. Los servicios estacionales presentan correlaciones débiles con otras prácticas, sugiriendo independencia en su aplicación.

```{r}
#| label: fig-management-correlation-matrix
#| fig-cap: "Correlaciones entre prácticas de gestión, mostrando que senderos gestionados, vallado y limpieza mecánica se implementan de forma coordinada, mientras que servicios estacionales operan independientemente."
#| echo: false
#| eval: true
#| output: true
#| fig-width: 10
