...
# Join the dominant species data with the NMDS data to associate species with clusters
dominant_with_clusters <- all_dominant_species %>%
  left_join(
    nmds_data %>% 
      mutate(row_id = row_number()) %>%
      select(row_id, Habitat),
    by = c("plot" = "row_id")
  )
...
indicator_species <- all_dominant_species %>%
  mutate(row_id = row_number()) %>%
  left_join(
    nmds_data %>% select(Habitat, Cluster),
    by = c("row_id" = "row_number")
  ) %>%
...
The NMDS analysis successfully classified the vegetation plots into four distinct habitat types, which align with typical zonation patterns in Mediterranean coastal dune systems:

1. **Front-dune Habitat**: Characterized by pioneer species adapted to harsh conditions at the seaward edge of the dune system. These areas experience high salt spray, sand movement, and limited soil development. Indicator species include *Cakile maritima*, *Elymus farctus*, and *Ammophila arenaria*.

2. **Back-dune Habitat**: Represents more stabilized areas further inland, with reduced wind and salt stress, and more developed soils. This habitat supports a greater diversity of species and is characterized by indicators such as *Crucianella maritima*, *Pancratium maritimum*, and *Helichrysum stoechas*.

3. **Mixed Habitat**: Transition zones between front and back dunes, or areas with overlapping characteristics from multiple habitat types. These areas often host a mixture of species from adjacent habitats and may represent successional stages.

4. **Disturbed Habitat**: Areas showing signs of human impact, characterized by the presence of ruderal or opportunistic species. These zones may be affected by human trampling, proximity to urban areas, or modified sediment dynamics due to coastal infrastructure.

The stress value of `r round(nmds_result$stress, 3)` indicates a `r if(nmds_result$stress < 0.1) "good" else if(nmds_result$stress < 0.2) "acceptable" else "poor"` fit of the NMDS ordination. Values below 0.2 are generally considered acceptable in ecological studies, suggesting that our two-dimensional representation captures the underlying ecological patterns adequately.

### Regional Distribution of Habitat Types

The analysis reveals notable differences in habitat distribution across the three study regions:

1. **Tarragona**: Shows the highest proportion of front-dune habitats, suggesting well-preserved embryonic dune systems. This may reflect the wider beaches and more natural coastal dynamics in this region.

2. **Barcelona**: Displays the highest percentage of disturbed habitats, consistent with the intense urbanization and tourism pressure in this densely populated coastal area.

3. **Girona**: Presents a more balanced distribution of habitat types, with substantial representation of back-dune and mixed habitats. This suggests more mature and complex dune systems.

These patterns align with the known differences in coastal development, protection status, and natural features across the Catalan coast, with the Barcelona area being highly urbanized, while Tarragona and Girona retain more natural coastal landscapes.

### Implications for Conservation and Management

The habitat classification from this NMDS analysis provides valuable insights for targeted conservation strategies:

- **Front-dune Habitats**: Require protection from trampling and coastal erosion, which are particularly vulnerable due to their position at the land-sea interface.

- **Back-dune Habitats**: Need management approaches that maintain habitat complexity and prevent fragmentation by recreational infrastructure.

- **Mixed Habitats**: May benefit from targeted interventions to ensure natural succession processes can continue.

- **Disturbed Habitats**: Present opportunities for restoration efforts, particularly in Barcelona where this habitat type dominates.

By tailoring management approaches to the specific needs of each habitat type identified through this analysis, conservation efforts can be more effective in preserving the biodiversity and ecological function of Catalonian coastal dune systems.
```