...
#### Análisis de Correspondencias Canónicas con Variables de Gestión por Región y Especies

Este capítulo presenta el CCA examinando las relaciones entre la composición de vegetación de dunas costeras y las prácticas de gestión de playas en las tres regiones: Girona, Barcelona y Tarragona.

**Aspectos clave de este análisis:**

1. **Variables de Gestión**: Variables de escala ordinal (0-5) que representan la calidad de gestión:
   - 0 = Sin gestión / Calidad pobre
   - 5 = Gestión de buena calidad

2. **Análisis Regional**: Análisis separado para cada región para entender la efectividad de gestión local

3. **Respuesta de Especies**: Cómo diferentes especies de plantas responden a varias intervenciones de gestión

**Variables de Gestión Analizadas:**
- Senderos gestionados (control del pisoteo)
- Vallados con cuerdas (protección de vegetación)
- Limpieza mecánica (mantenimiento de playa)
- Impacto de servicios estacionales
- Impacto de infraestructura fija
...
prepare_mgmt_matrix <- function(management_region_data, beach_ids) {
  ...
  # Identify potential management variables (numeric columns, excluding all ID columns)
  id_columns <- c("id_beach", "id_plot", "plot", "id_transect")
  ...
  # Clean up column names for better readability using global translation dictionary
  ...
  # Apply translations for known variables
  ...
  # For any remaining untranslated variables, use the global translation system
  # but only if they haven't been translated already
  remaining_vars <- clean_names[!clean_names %in% name_mappings]
  if(length(remaining_vars) > 0) {
    # Check if any variables are still in English (contain common English words)
    ...
    if(length(english_vars) > 0) {
      # Try to translate English variables using the global system
      tryCatch({
        translated_english <- translate_labels(english_vars, debug = FALSE)
        # Replace the English variables with their translations
        for(i in seq_along(english_vars)) {
          clean_names[clean_names == english_vars[i]] <- translated_english[i]
        }
      }, error = function(e) {
        # If translation fails, use cleaned version
        warning(paste("Could not translate some management variables:", paste(english_vars, collapse = ", ")))
...
cca_mgmt_results$combinado <- run_cca_mgmt_analysis(all_species_mgmt_data, all_mgmt_data, "Combinado")
...
combinado_cca_mgmt <- cca_mgmt_results$combinado
```
...
create_cca_mgmt_biplot <- function(cca_analysis, title_suffix = "") {
  ...
  p <- ggplot() +
    ...
    theme_minimal() +
    labs(
      title = paste("Biplot CCA - Variables de Gestión:", cca_analysis$region, title_suffix),
      subtitle = paste("Varianza explicada:", round(cca_analysis$explained_variance, 1), "% | Escala de gestión: 0 (ninguna) a 5 (buena)"),
...
create_mgmt_importance_plot <- function(cca_results) {
  ...
  p <- ggplot(mgmt_importance, aes(x = reorder(Variable, Importance), y = Importance, fill = Region)) +
    ...
    labs(
      title = "Importancia de Variables de Gestión en el Análisis CCA",
      subtitle = "La longitud de las flechas del biplot CCA indica importancia",
      x = "Variables de Gestión",
      y = "Importancia (Longitud de Flecha)",
      fill = "Región"
    ) +
    theme_minimal() +
    theme(
...
El CCA revela relaciones débiles entre prácticas de gestión y composición vegetal (@fig-cca-mgmt-combinado). Los vectores de gestión muestran direcciones dispersas sin agrupaciones claras, indicando que ninguna práctica de manejo estructura consistentemente las comunidades vegetales. Las especies como *Eryngium* y *Plantago* aparecen asociadas débilmente con gradientes específicos de gestión, pero la varianza explicada total sugiere efectos limitados del manejo sobre la vegetación dunar.

```{r}
#| label: fig-cca-mgmt-combinado
#| fig-cap: "Biplot CCA combinado mostrando relaciones dispersas entre gestión y vegetación, indicando efectos débiles de las prácticas de manejo sobre la composición específica en sistemas dunares costeros."
#| echo: false
#| eval: true
#| output: true
#| fig-width: 15
...
# Create and display the combinado CCA biplot with management variables
if(!is.null(combinado_cca_mgmt)) {
  p_combinado_mgmt <- create_cca_mgmt_biplot(combinado_cca_mgmt, "- Todas las Regiones")
  print(p_combinado_mgmt)
  
  # Save the plot
  ggsave(here("figures","cca_combined_management_analysis.png"), p_combinado_mgmt, width = 12, height = 10)
} else {
  cat("No se pudo realizar el análisis CCA combinado con variables de gestión.\n")
  cat("Posibles razones:\n")
  cat("- Datos insuficientes coincidentes entre conjuntos de datos de especies y gestión\n")
  cat("- No hay variables de gestión con variación suficiente\n")
  cat("- Problemas técnicos con la preparación de datos\n")
}
```

Los análisis CCA regionales revelan patrones diferenciados de efectividad de gestión (@fig-cca-mgmt-regional, @fig-mgmt-importance). Girona muestra la menor varianza explicada (42.3%) con vectores de gestión dispersos, indicando efectos débiles. Barcelona presenta alta varianza explicada (100%) pero con solo dos variables activas (senderos gestionados y servicios estacionales), reflejando su limitado rango de gestión. Tarragona exhibe varianza moderada (50.5%) con vectores de gestión más agrupados, sugiriendo asociación entre las prácticas de manejo. Los análisis de significancia confirman efectos generalmente débiles, con solo senderos gestionados en Tarragona alcanzando significancia estadística (p < 0.05).

```{r}
#| label: fig-cca-mgmt-regional
#| fig-cap: "CCA regionales mostrando efectividad diferencial de gestión: Girona con efectos débiles, Barcelona con rango limitado, y Tarragona con mayor coherencia en prácticas de manejo."
#| echo: false
#| eval: true
#| output: true
#| fig-width: 15
...
if(length(regional_mgmt_plots) > 0) {
  ...
  if(length(valid_mgmt_plots) > 0) {
    if(length(valid_mgmt_plots) == 3) {
      combinado_regional_mgmt <- valid_mgmt_plots$girona / valid_mgmt_plots$barcelona / valid_mgmt_plots$tarragona
    } else if(length(valid_mgmt_plots) == 2) {
      combinado_regional_mgmt <- valid_mgmt_plots[[1]] / valid_mgmt_plots[[2]]
    } else {
      combinado_regional_mgmt <- valid_mgmt_plots[[1]]
    }
    
    print(combinado_regional_mgmt)
    
    # Save the combinado regional plot
    ggsave(here("figures","cca_regional_management_analysis.png"), combinado_regional_mgmt,
           width = 14, height = 16)
  } else {
    cat("No se pudieron crear gráficos CCA regionales válidos.\n")
  }
} else {
  cat("No se pudieron realizar análisis CCA regionales con variables de gestión.\n")
}
```

```{r}
#| label: fig-mgmt-importance
#| fig-cap: "Importancia de variables de gestión entre regiones, mostrando qué prácticas de gestión tienen la mayor influencia en la composición de vegetación. Barras más largas indican prácticas de gestión más influyentes."
#| echo: false
#| eval: true
#| output: true
#| fig-width: 12
...
if(length(valid_results) > 0) {
  ...
  if(!is.null(p_importance)) {
    ...
    # Save the plot
    ggsave(here("figures","management_variable_importance.png"), p_importance, width = 12, height = 8)
  } else {
    cat("No se pudo crear el gráfico de importancia de gestión.\n")
  }
} else {
  cat("No hay resultados CCA válidos disponibles para el análisis de importancia.\n")
}
```
...
# Check each region and combinado analysis
all_regions <- c("girona", "barcelona", "tarragona", "combinado")
...
knitr::kable(setNames(results_summary[, c("Region", "Explained_Variance", "Species_Count", "Beach_Count")],
c("Región", "Varianza Explicada (%)", "Número de Especies", "Número de Playas")),
      caption = "Resumen CCA: Varianza Explicada, Número de Especies y Playas",
      format = "latex", booktabs = TRUE) %>%
  kable_styling(latex_options = "hold_position")

knitr::kable(setNames(results_summary[, c("Region", "Management_Variables")], c("Región", "Variables descriptivas para CCA")),
      caption = "Variables de Gestión por Región",
      format = "latex", booktabs = TRUE) %>%
  kable_styling(latex_options = "hold_position") %>%
...
cat("\nAnálisis exitosos:", successful_analyses, "de", nrow(results_summary), "\n")
```
...
for(region_name in names(valid_results)) {
  if(!is.null(valid_results[[region_name]])) {
    cat("Probando significancia para la región", str_to_title(region_name), ":\n")
    ...
    if(!is.null(sig_test$model)) {
      ...
      if(!is.na(sig_test$model$F[1]) && !is.na(sig_test$model$`Pr(>F)`[1])) {
        cat("  Modelo general F =", round(sig_test$model$F[1], 3),
        ...
        cat("  No se pudo completar la prueba de significancia del modelo general\n")
      }
    } else {
      cat("  Falló la prueba de significancia del modelo general\n")
    }
    
    if(!is.null(sig_test$variables)) {
      cat("  Significancia de variables individuales:\n")
      for(i in 1:nrow(sig_test$variables)) {
        if(!is.na(sig_test$variables$F[i]) && !is.na(sig_test$variables$`Pr(>F)`[i])) {
          ...
          var_name <- rownames(sig_test$variables)[i]
          cat("    ", var_name, ": falló la prueba de significancia\n")
        }
      }
    } else {
      cat("  Fallaron las pruebas de significancia de variables individuales\n")
    }
    cat("\n")
  }
}
...
#| tbl-cap: "Resumen de efectos de variables de gestión entre regiones."
#| echo: false
#| eval: true
#| output: true
#| tbl-pos: "H"
...
if(length(valid_results) > 0) {
  ...
  for(region_name in names(valid_results)) {
    if(!is.null(valid_results[[region_name]])) {
      ...
      temp_df <- data.frame(
        ...
        P_Value_with_Sig = ifelse(is.na(p_values), "NA",
                                 paste0(round(p_values, 4),
                                       ifelse(p_values < 0.001, " ***",
                                             ifelse(p_values < 0.01, " **",
                                                   ifelse(p_values < 0.05, " *",
                                                         ifelse(p_values < 0.1, " .", " (n.s.)")))))),
        stringsAsFactors = FALSE
      )
  ...
  if(nrow(mgmt_effects_summary) > 0) {
    ...
    mgmt_effects_summary$P_Value <- ifelse(is.na(mgmt_effects_summary$P_Value), "NA", as.character(mgmt_effects_summary$P_Value))
    
    display_table <- mgmt_effects_summary %>%
      select(Region, Management_Variable, Importance, P_Value_with_Sig)

    knitr::kable(display_table,
                 col.names = c("Región", "Variable de Gestión", "Importancia", "Valor P"),
                 format = "latex",
                 booktabs = TRUE,
                 row.names = FALSE) %>%
    ...
    kableExtra::footnote(
      general = c("Códigos de significancia: *** p<0.001, ** p<0.01, * p<0.05, . p<0.1, (n.s.) = no significativo",
                  "NA = No aplicable debido a sobreajuste estadístico (Barcelona: 4 playas, 7 variables)"),
      general_title = "Nota:",
      escape = FALSE
    )
  } else {
    cat("No se pudo crear el resumen de efectos de gestión.\n")
  }
} else {
  cat("No hay resultados válidos disponibles para el resumen de efectos de gestión.\n")
}
```
