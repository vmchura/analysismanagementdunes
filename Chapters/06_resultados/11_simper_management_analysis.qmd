...
validate_group_sizes <- function(mgmt_groups, min_size = 3) {
...
prepare_complete_simper_dataset <- function(region_data, mgmt_data, region_name) {
  ...
  if(length(valid_indices) < 6) {  # Mínimo 6 playas para análisis
...
#| tbl-cap: "Resumen de la preparación de datos para análisis SIMPER, mostrando el número de playas, especies y grupos de gestión por región."
#| echo: false
#| eval: true
#| output: true

if(exists("preparation_summary") && nrow(preparation_summary) > 0) {
  knitr::kable(preparation_summary,
               col.names = c("Región", "Estado", "Playas", "Especies", "Grupos de Gestión"),
               caption = "Resumen de Preparación de Datos SIMPER",
               format = "latex", 
               booktabs = TRUE) %>%
    kable_styling(latex_options = "hold_position") %>%
    column_spec(5, width = "6cm") %>%
    row_spec(which(preparation_summary$Status == "OK"), color = "black") %>%
    row_spec(which(preparation_summary$Status == "ERROR"), color = "red")
} else {
...
3. **Validación de grupos**: Verificación de tamaños mínimos (≥3 playas) para análisis estadístico robusto
...
if(length(valid_simper_datasets) > 0) {
  ...
  # Guardar gráfico
  ggsave(here("figures","management_distribution_by_region.png"), width = 12, height = 8)
...
if(length(valid_simper_datasets) > 0) {
  ...
  if(nrow(intensity_data) > 0) {
    ...
    # Guardar gráfico
    ggsave(here("figures","management_intensity_distribution.png"), width = 12, height = 6)
  } else {
...
run_simper_analysis <- function(simper_dataset, group1, group2) {
  ...
  if(length(group1_indices) < 3 || length(group2_indices) < 3) {
  ...
  tryCatch({
    ...
    return(list(
      ...
      species_contributions = simper_data
    ))
  }, error = function(e) {
...
extract_discriminant_species <- function(simper_result, min_contrib = 5, min_ratio = 1.5, min_presence = 30) {
  ...
  discriminant_species <- species_data %>%
    filter(
      contr >= min_contrib,          # Contribución mínima 5%
      contr/sd >= min_ratio,         # Ratio contribución/SD >= 1.5
      pmax(p1, p2) >= min_presence   # Presencia mínima 30% en al menos un grupo
    ) %>%
    arrange(desc(contr))
...
for(region_name in names(valid_simper_datasets)) {
  ...
  if(!is.null(pairwise_results) && length(pairwise_results) > 0) {
    ...
    for(comparison_name in names(pairwise_results)) {
      ...
      cat(sprintf("  %s: Disimilitud = %.2f%%\n", 
                  comparison_name, comparison$dissimilarity))
    }
  } else {
...
if(exists("simper_summary") && nrow(simper_summary) > 0) {
  # Crear tabla resumida para presentación
  display_summary <- simper_summary %>%
    select(Region, Group1_Spanish, Group2_Spanish, Dissimilarity, Top_Species, Top_Contribution, N_Discriminant) %>%
    arrange(Region, desc(Dissimilarity))
  
  knitr::kable(display_summary,
               col.names = c("Región", "Grupo 1", "Grupo 2", "Disimilitud (%)", 
                           "Especie Principal", "Contribución (%)", "N° Discriminantes"),
               caption = "Especies discriminantes: contribución ≥5%, ratio ≥1.5, presencia ≥30%",
               format = "latex", 
               booktabs = TRUE) %>%
