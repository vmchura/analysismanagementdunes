...
extract_discriminant_species <- function(simper_result, min_contrib = 2, min_ratio = 1.0, min_presence = 10) {
  ...
  tryCatch({
    ...
    if (interactive()) cat("Original species count:", nrow(discriminant_df), "\n")
    ...
    if (interactive()) cat("After contribution filter (>=", min_contrib, "%):", nrow(step1), "\n")
    
    # Paso 2: Filtrar por ratio (más permisivo)
    step2 <- step1 %>%
      filter(!is.na(sd) & sd > 0 & contr/sd >= min_ratio)
    if (interactive()) cat("After ratio filter (>=", min_ratio, "):", nrow(step2), "\n")
    
    # Paso 3: Filtrar por presencia (más permisivo)
    step3 <- step2 %>%
      filter(!is.na(p1) & !is.na(p2) & pmax(p1, p2, na.rm = TRUE) >= min_presence)
    if (interactive()) cat("After presence filter (>=", min_presence, "):", nrow(step3), "\n")
    
    # Si todavía no hay especies, usar criterios muy relajados
    if(nrow(step3) == 0) {
      if (interactive()) cat("No species meet all criteria, using most relaxed filters...\n")
      ...
      if (interactive()) cat("Relaxed criteria result:", nrow(relaxed_result), "species\n")
      if(nrow(relaxed_result) > 0) {
        if (interactive()) cat("Top species:", paste(head(relaxed_result$species, 3), collapse = ", "), "\n")
        if (interactive()) cat("Top contributions:", paste(round(head(relaxed_result$contr, 3), 2), collapse = ", "), "%\n")
      }
...
summarize_simper_results <- function(simper_comparisons) {
  ...
  for(comparison_name in names(simper_comparisons)) {
    ...
    if(!is.null(discriminant_spp) && nrow(discriminant_spp) > 0) {
      if (interactive()) cat("Successfully found", nrow(discriminant_spp), "discriminant species\n")
      ...
      if (interactive()) cat("No discriminant species found for this comparison\n")
    }
  }
...
if (interactive()) cat("Ejecutando análisis SIMPER por pares...\n")
...
for(region_name in names(valid_simper_datasets)) {
  ...
  if(!is.null(pairwise_results) && length(pairwise_results) > 0) {
    ...
    if (interactive()) cat(sprintf("Comparaciones exitosas: %d\n", length(pairwise_results)))
    ...
    if (interactive()) cat("No se pudieron realizar comparaciones SIMPER\n")
  }
}
...
if (interactive()) cat(sprintf("Total de comparaciones realizadas: %d\n", total_comparisons))
if (interactive()) cat(sprintf("Regiones con análisis exitosos: %d\n", length(simper_results)))
...
La @tbl-simper-species muestra las especies que más contribuyen a caracterizar cada región, destacando su importancia relativa en la diferenciación de las comunidades vegetales según el nivel de gestión aplicado en las playas.

Los resultados del análisis SIMPER (@tbl-simper-summary) indican una disimilitud muy baja entre los grupos de gestión (0.1%), lo que sugiere diferencias poco significativas en la composición vegetal asociada a las distintas prácticas de manejo.

La @fig-simper-contributions detalla las contribuciones específicas de cada especie discriminante, tanto por región como por comparación entre grupos.

A pesar del bajo nivel de disimilitud general, el análisis SIMPER revela que ciertas especies responden de manera diferencial a la intensidad de gestión, lo que las convierte en indicadores biológicos potenciales para evaluar el impacto de las prácticas de manejo costero.

Las muestras de Barcelona no se consideraron en este análisis porque no existen suficientes muestras con variedad de intensidad de prácticas de gestión.

```{r}
#| label: tbl-simper-species
#| tbl-cap: "Top 3 especies únicas con mayor contribución a la discriminación entre grupos por región"
#| echo: false
#| eval: true
#| output: true
#| tbl-pos: "H"
...
if(exists("simper_summary") && nrow(simper_summary) > 0) {
  # Crear tabla separada para especies - evitar duplicados
  species_summary <- simper_summary %>%
    select(Region, Top_Species, Top_Contribution) %>%
    # Agregar por región y especie para obtener la máxima contribución
    group_by(Region, Top_Species) %>%
    summarise(Max_Contribution = max(Top_Contribution, na.rm = TRUE), .groups = "drop") %>%
    # Ordenar y tomar top 3 especies únicas por región
    arrange(Region, desc(Max_Contribution)) %>%
    group_by(Region) %>%
    slice_head(n = 3) %>%  # keep only top 3 unique species per region
    ungroup() %>%
    mutate(
      Especie = ifelse(nchar(Top_Species) > 26,
                      paste0(substr(Top_Species, 1, 24), "..."),
                      Top_Species),
      Contrib = round(Max_Contribution, 1)
    ) %>%
    select(Region, Especie, Contrib)

  knitr::kable(species_summary,  # Show top 3 unique species per region
...
#| tbl-cap: "Resumen de análisis SIMPER mostrando disimilitud entre grupos de gestión y especies discriminantes principales por región. SG=Sin Gestión, GB=Gestión Baja, GM=Gestión Moderada, GA=Gestión Alta"
#| echo: false
#| eval: true
#| output: true
#| results: asis
#| tbl-pos: "H"
if (exists("simper_summary") && nrow(simper_summary) > 0) {
  # Crear tabla de resumen con solo 3 comparaciones por región
  display_summary <- simper_summary %>%
    ...
    distinct(Region, Group1_Spanish, Group2_Spanish, .keep_all = TRUE) %>%
    group_by(Region) %>%
    ungroup() %>%
    mutate(
      G1 = case_when(
  ...
  knitr::kable(display_summary,
               col.names = c("Región", "Comparación", "Disimilitud (%)", "N° Especies"),
               format = "latex",
               booktabs = TRUE) %>%
    kable_styling(latex_options = c("HOLD_position", "scale_down"),
                  full_width = FALSE,
