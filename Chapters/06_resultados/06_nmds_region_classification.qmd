...
Esta sección presenta un análisis de NMDS que compara las comunidades
vegetales entre las tres regiones de Cataluña: Tarragona, Barcelona y Girona. Este análisis explora cómo los patrones de
vegetación de dunas costeras difieren entre estas regiones e identifica las especies características de cada área.

La @fig-nmds-region-plot muestra la distribución espacial de las parcelas de vegetación según su composición florística,
revelando patrones regionales distintivos en las comunidades de dunas costeras catalanas.
...
La @fig-region-species-indicators identifica las especies más representativas de cada región basándose en su abundancia media según la escala de Braun-Blanquet. Este análisis permite reconocer los elementos florísticos distintivos que caracterizan las dunas costeras de cada provincia catalana.
```{r}
#| label: fig-region-species-diversity
#| fig-cap: "Métricas de diversidad de especies por región, mostrando riqueza, índice de diversidad de Shannon y equitatividad en Tarragona, Barcelona y Girona."
#| echo: false
#| eval: true
#| output: true
#| fig-width: 12
...
diversity_long <- diversity_data %>%
  pivot_longer(
    cols = c(Riqueza, Shannon, Equitatividad),
    names_to = "Métrica",
    values_to = "Valor"
  )
...
ggplot(diversity_long, aes(x = Region, y = Valor, fill = Region)) +
  ...
  theme_minimal() +
  labs(
    title = "Métricas de Diversidad por Región",
    y = "Valor",
    x = "Región"
  ) +
  # Use consistent region colors
...
### Diversidad Florística Regional

La @fig-region-species-diversity presenta tres métricas complementarias de diversidad que caracterizan la riqueza florística de las dunas costeras en cada región:

- **Riqueza**: Número total de especies presentes en cada región
- **Índice de Shannon**: Medida que incorpora tanto la riqueza como la abundancia relativa de especies
- **Equitatividad**: Grado de uniformidad en la distribución de abundancias entre especies

Estas métricas proporcionan una visión integral de la complejidad florística regional y permiten identificar áreas de particular valor para la conservación de la biodiversidad dunar catalana.

## Conclusiones del Análisis Regional

El análisis NMDS revela una clara diferenciación florística entre las tres regiones costeras de Cataluña, con patrones que reflejan tanto gradientes ambientales naturales como el impacto de diferentes presiones antrópicas. Esta diferenciación regional tiene importantes implicaciones para:

1. **Conservación**: Identificación de especies y comunidades prioritarias en cada región
2. **Gestión**: Desarrollo de estrategias de manejo adaptadas a las características florísticas locales  
3. **Restauración**: Selección de especies apropiadas para proyectos de rehabilitación dunar
4. **Monitoreo**: Establecimiento de indicadores regionales específicos para el seguimiento de la biodiversidad
```