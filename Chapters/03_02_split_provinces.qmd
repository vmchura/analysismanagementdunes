...
The data splitting process builds upon the processed data from Chapter 1. We use the main dataset with species observations to create the regional subsets.
...
First, we load the saved processed data from the previous chapter:
...
## Data Characteristics by Region

To understand the distribution of data across regions, we analyze key characteristics of each regional dataset:

```{r}
#| label: regional-characteristics
#| echo: true
#| eval: true
# Function to summarize region characteristics
summarize_region <- function(region_name, region_data) {
  unique_beaches <- length(unique(region_data$id_beach))
  unique_plots <- length(unique(region_data$plot))
  
  cat(region_name, "characteristics:\n")
  cat("- Unique beaches:", unique_beaches, "\n")
  cat("- Unique plots:", unique_plots, "\n")
  cat("- Observations:", nrow(region_data), "\n\n")
}

# Apply the summary function to each region
for(region_name in names(beaches_by_region)) {
  summarize_region(region_name, beaches_by_region[[region_name]])
}
```
...
1. Loaded the processed data from Chapter 1
...
5. Analyzed key characteristics of each regional dataset
6. Visualized the distribution of observations across regions
7. Saved the regional datasets for subsequent analyses
